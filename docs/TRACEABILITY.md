...
| FR-0.5 | ADR-002 static PMTiles | `GeoJsonExporter` (`export-geojson`) ✓ · `tools/build-tiles.sh` + Blob publish (next) | Reproducible-build check; smoke test loads tiles; interaction check on mid-range laptop | In progress |
...
| NFR-7 | [DESIGN.md](./DESIGN.md) a11y section | Native inputs, focus ring, aria-live KPIs (Week 4) | Automated a11y scan on chrome; keyboard walkthrough in demo script | Planned |
