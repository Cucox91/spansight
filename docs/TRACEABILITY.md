...
|---|---|---|---|---|
| FR-1.1 | ADR-005 Parquet + DuckDB · ARCH §4.2 | **Foundation merged PR #24:** `SpanSight.Core/Vintages` (superset schema, era classifier, fail-loud header bind, reject codes) + `convert-vintage` CLI command + `tools/vintages/{download,convert}.sh` + committed `catalog.json` + `tools/vintages/README.md`. Remaining: all 34 vintages converted, Blob cool-tier archive script (P1-W2) | Three era samples convert clean and reconcile exactly — 1992 666,206 · 2010 604,493 · 2025 624,193 = **1,894,892 rows in = converted + 0 rejected**, recorded in `catalog.json` ✓; 300-row fixtures for all three eras convert end-to-end in CI (.NET + DuckDB) ✓; 41 unit/golden tests incl. wrong-era fail-loud, superset NULL-fill, reject codes ✓; clean-checkout reproduction documented (AC-4) ✓. Full 34-vintage reconciliation = P1-W2 | In progress (foundation done PR #24; W2 completes) |
