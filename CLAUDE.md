...
Next: **[RAZIEL] RUNBOOK §1** (PAYG, `spansight-deploy` app + OIDC federation, GitHub secrets/vars, PG Entra admin params) → first Deploy run → RUNBOOK §1.4 PG principals + §3 cloud data load + tile publish + `VITE_TILES_URL` → `run_e2e` live smoke · then the 0.5 gate: [RAZIEL] Anthropic key via user-secrets/Container Apps secrets + live-key smoke + `Ai:Enabled` flip (FR-AI.2/3 remain Could-priority stubs) · Raziel's post-completion code study begins at Phase 0 gate.
