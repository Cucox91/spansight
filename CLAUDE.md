...
Next: **[RAZIEL] RUNBOOK §1** (PAYG, `spansight-deploy` app + OIDC federation, GitHub secrets/vars, PG Entra admin params) → first Deploy run → RUNBOOK §1.4 PG principals + §3 cloud data load + tile publish + `VITE_TILES_URL` → `run_e2e` live smoke · then the Phase 0.5 AI series (FR-AI.1 first; `Ai:Enabled` false until its gate; Anthropic key via .env/secrets only) · Raziel's post-completion code study begins at Phase 0 gate.
